...
//         }
//     }
//     // std::cout << "更新后的中心点数量: " << centers.size() << std::endl;
//     // for (size_t i = 0; i < centers.size(); ++i)
//     // {
//     //     std::cout << "中心点 " << i << ": x: " << centers[i].point.x() 
//     //               << ", y: " << centers[i].point.y() 
//     //               << ", d: " << centers[i].diameters << std::endl;
//     // }
//     return centers;
// }
...
//分配每个数据点到最近的簇
void allocate_centers(std::vector<Circles>& samples, const std::vector<Circles>& centers)
{
    for (auto& sample: samples)
    {
        double min_dist = distance(sample, centers[0]);
        sample.cluster_id = 0;
        for(size_t j = 1; j < centers.size(); ++j)
        {
            double dist = distance(sample, centers[j]);
            if (min_dist > dist)
            {
                min_dist = dist;
                sample.cluster_id = j;
...
vector<Circles> computeClusterCentersInOriginalSpace(
    ...
    // std::cout << "聚类中心点数量: " << centers.size() << std::endl;
    // for (size_t i = 0; i < centers.size(); ++i) 
    // {
    //     std::cout << "聚类中心点 " << i << ": x: " << centers[i].point.x() 
    //               << ", y: " << centers[i].point.y() 
    //               << ", d: " << centers[i].diameters Noramalize_Clustering<< std::endl;
    // }
...
std::vector<Circles> Clustering(std::vector<Circles> samples)
{
    ...
    // 第一步：分配每个点到最近的簇
    allocate_centers(normalize_data, Noramalize_Clustering);
    // 第二步：更新中心点
    // Noramalize_Clustering = calculate_center(normalize_data);
    // 第三步：将标准化后的聚类结果映射回原始空间
    std::vector<Circles> Clustering_Result = computeClusterCentersInOriginalSpace(normalize_data, samples);//Clustering_Result用于储存聚类结果
