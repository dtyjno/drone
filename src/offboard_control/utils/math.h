...
// 将函数改为内联函数避免重复定义
inline bool is_zero(float _ki) {
    return fabsf(_ki) < 1.0e-6f;
}

inline bool is_positive(float dt) {
    return dt >= 1.0e-6f;
}

inline bool is_negative(float _error) {
    return _error <= -1.0e-6f;
}
...
inline bool is_equal(Vector4f a, Vector4f b, float tolerance = 0.001) {
    return (a - b).norm() < tolerance;
}

inline float constrain_float(float _integrator, float _max, float _min) {
    if (_integrator > _max) return _max;
    if (_integrator < _min) return _min;
    return _integrator;
}
