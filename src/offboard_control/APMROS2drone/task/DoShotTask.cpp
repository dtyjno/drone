...
bool DoShotTask::init(DeviceType device) {
    ...
    if constexpr (!std::is_same_v<DeviceType, std::shared_ptr<APMROS2Drone>>) {
        // device->log_error("%s: DoShotTask::init can only be used with APMROS2Drone", get_string().c_str());
    ...
    if constexpr (!std::is_same_v<DeviceType, std::shared_ptr<APMROS2Drone>>) {
        ...
        if (shot_flag) 
        {
            ...
            if (find_duration >= shot_duration + shot_wait) {
                device->log_info("Doshot: 投弹后等待, find_duration_time = %fs", find_duration);
                task_result = true;
                return true;
            }
        }
                ...
                !device->get_yolo_detector()->is_get_target(YOLO_TARGET_TYPE::CIRCLE))
        {
            ...
            if (task->get_task_result())
            {
                ...
                device->log_info_throttle(std::chrono::milliseconds(100), "(T 0.1s) Doshot: Approach, Doshot, time = %fs", find_duration);
            ...
            else if (!shot_flag)
            {
                if (waypoint_task != nullptr) {
                    device->log_info_throttle(std::chrono::milliseconds(100), "%s: (T 0.1s) Doshot: Searching for target, time = %fs", get_string().c_str(), find_duration);
                    waypoint_task->execute_apm(device);
