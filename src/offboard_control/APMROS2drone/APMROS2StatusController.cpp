...
void APMROS2StatusController::init_ROS2_topics() {
	// 质量服务配置
	...
	arm_motors_client_ = node->create_client<mavros_msgs::srv::CommandBool>(topic_namespace+"cmd/arming");
	mode_switch_client_ = node->create_client<mavros_msgs::srv::SetMode>(topic_namespace+"set_mode");
	set_home_client_ = node->create_client<mavros_msgs::srv::CommandHome>(topic_namespace+"cmd/set_home");
	takeoff_client_ = node->create_client<mavros_msgs::srv::CommandTOL>(topic_namespace+"cmd/takeoff");
	land_client_ = node->create_client<mavros_msgs::srv::CommandTOL>(topic_namespace+"cmd/land");
	param_set_client_ = node->create_client<mavros_msgs::srv::ParamSetV2>(topic_namespace+"param/set");

	
	state_subscription_ = node->create_subscription<mavros_msgs::msg::State>(topic_namespace+"state", qos,
		std::bind(&APMROS2StatusController::state_callback, this, std::placeholders::_1),sub_opt);
	// * /mavros/home_position/home [mavros_msgs/msg/HomePosition] 1 subscriber
	home_position_subscription_ = node->create_subscription<mavros_msgs::msg::HomePosition>(topic_namespace+"home_position/home", qos,
		std::bind(&APMROS2StatusController::home_position_callback, this, std::placeholders::_1),sub_opt);
}

// bool APMROS2StatusController::armed;
// bool APMROS2StatusController::connected;
// bool APMROS2StatusController::guided;
// std::string APMROS2StatusController::mode;
// std::string APMROS2StatusController::system_status;
// Vector3f APMROS2StatusController::home_position = Vector3f::Zero();
// Vector3f APMROS2StatusController::home_position_global = Vector3f::Zero();
// Quaternionf APMROS2StatusController::home_quaternion = Quaternionf::Identity(); // 使用四元数类
...
// 5. 起飞
// 6. 若未解锁则回到第3步
bool APMROS2StatusController::takeoff(float local_frame_z,float takeoff_altitude, float yaw){
	...
	case TakeoffState::init :
		...
		switch_mode("GUIDED");
		state_ = TakeoffState::wait_for_takeoff_command;
		break;
	case TakeoffState::wait_for_takeoff_command :
		RCLCPP_INFO_ONCE(node->get_logger(), "解锁前所有准备已完成，按下回车解锁无人机");
		if(takeoff_command == true) {
			RCLCPP_INFO(node->get_logger(), "开始解锁");
			// drone->set_start_time(drone->get_cur_time()); // 设置开始时间
			arm_motors(true);
			timer = new Timer(); // 重置计时器
			state_ = TakeoffState::arm_requested;	
		}
		break;
	case TakeoffState::wait_for_stable_offboard_mode :
		if (++num_of_steps>10){
			arm_motors(true);
			state_ = TakeoffState::arm_requested;
		}
		break;
	case TakeoffState::arm_requested : // skip
		if(!armed){//_arm_done
	...
	case TakeoffState::takeoff:
		...
		if (!armed){ // 如果无人机起飞失败重新上锁
			...
			state_ = TakeoffState::end;
		}
		break;
	case TakeoffState::end:
		is_takeoff = true;
		delete timer; // 删除计时器对象
		timer = nullptr; // 设置指针为空，避免悬挂指针
		state_ = TakeoffState::init; // 重置状态
		break;
	default:
		break;
	}
...
void APMROS2StatusController::state_callback(const mavros_msgs::msg::State::SharedPtr msg) 
{
	armed = msg->armed;
...
// 接收home位置数据
void APMROS2StatusController::home_position_callback(const mavros_msgs::msg::HomePosition::SharedPtr msg) 
{
	home_position = {
...
*/
void APMROS2StatusController::switch_mode(const std::string& mode)
{
...
// void APMROS2StatusController::switch_to_auto_mode(){
// 	RCLCPP_INFO(node->get_logger(), "requesting switch to mode");
// 	APMROS2StatusController::switch_mode("AUTO");
// }


// arm or disarm motors
// arm= arm: true, disarm: false
void APMROS2StatusController::arm_motors(bool arm)
{
		...
		[node,this](rclcpp::Client<mavros_msgs::srv::CommandBool>::SharedFuture future) {
			...
			if (status == std::future_status::ready) {
				...
				if (reply) {
					...
					// Code to execute if the future is unsuccessful
					// _arm_done = false;
					RCLCPP_ERROR(node->get_logger(), ("Failed to call service " + topic_namespace + "cmd/arming").c_str());
				}
			} else {
				// Wait for the result.
				RCLCPP_INFO(node->get_logger(), "Service In-Progress...");
...
void APMROS2StatusController::set_home_position(float lat, float lon, float alt, float yaw)
{
		...
		[node,this](rclcpp::Client<mavros_msgs::srv::CommandHome>::SharedFuture future) {
			...
			if (status == std::future_status::ready) {
				...
				if (reply) {
					...
					// Code to execute if the future is unsuccessful
					RCLCPP_ERROR(node->get_logger(), ("Failed to call service " + topic_namespace + "cmd/set_home").c_str());
				}
			} else {
				// Wait for the result.
				RCLCPP_INFO(node->get_logger(), "Service In-Progress...");
...
void APMROS2StatusController::set_home_position(float yaw)
{
	auto request = std::make_shared<mavros_msgs::srv::CommandHome::Request>();
...
void APMROS2StatusController::command_takeoff_or_land(std::string mode, float altitude, float yaw)
{
			...
			[this,node](rclcpp::Client<mavros_msgs::srv::CommandTOL>::SharedFuture future) {
				...
				if (status == std::future_status::ready) {
					...
					if (reply == 1) {
						...
						// Code to execute if the future is unsuccessful
						RCLCPP_ERROR(node->get_logger(), ("Failed to call service "+topic_namespace+"cmd/takeoff").c_str());
					}
				} else {
					// Wait for the result.
			...
			[this,node](rclcpp::Client<mavros_msgs::srv::CommandTOL>::SharedFuture future) {
				...
				if (status == std::future_status::ready) {
					...
					if (reply == 1) {
						...
						// Code to execute if the future is unsuccessful
						RCLCPP_ERROR(node->get_logger(), ("Failed to call service "+topic_namespace+"cmd/land").c_str());
					}
				} else {
					// Wait for the result.
...
void APMROS2StatusController::set_param(const std::string& param_id, double value)
{
		...
		[node,this,param_id](rclcpp::Client<mavros_msgs::srv::ParamSetV2>::SharedFuture future) {
			...
			if (status == std::future_status::ready) {
				...
				if (reply) {
					...
					RCLCPP_ERROR(node->get_logger(), ("Failed to call service "+topic_namespace+"param/set").c_str());
				}
			} else {
				// Wait for the result.
