...
bool DoLandTask::init(DeviceType device) {
    ...
    if (device->get_camera_gimbal() == nullptr || device->get_servo_controller() == nullptr || device->get_yolo_detector() == nullptr) {
        ...
        device->log_info("返回降落点 x: ", x_home, "   y: ", y_home, "    angle: ", 0.0);
...
bool DoLandTask::run(DeviceType device) {
    ...
    if (device->get_camera_gimbal() == nullptr || device->get_servo_controller() == nullptr || device->get_yolo_detector() == nullptr) {
        ...
        if (parameters.dynamic_target_image_callback().data == Vector2f::Zero()) // yolo未识别到YOLO::TARGET_TYPE::H   (YOLO::TARGET_TYPE::CIRCLE)
        {
            ...
            {
                    device->log_info("Doland: surround_land = ", surround_land);
                    device->rotate_global2stand(scout_x + static_cast<double>(surround_land) * 1.0, scout_y, x_home, y_home);
                    device->log_info("Doland: land点 x: ", x_home, "   y: ", y_home, "   angle: ", 0.0); // 开始执行程序+x_home位置
                    device->send_start_setpoint_command(x_home, y_home, scout_halt, 0);
        ...
        {
            device->log_info_throttle(std::chrono::milliseconds(1000), "Doland: 看见H了，执行Doland");
            task->execute(device);
            ...
            {
                device->log_info_throttle(std::chrono::milliseconds(1000), "Doland: 未到达降落点");
