...
public:
	OffboardControl(std::string ardupilot_namespace) :
		...
		global_gps_publisher_{this->create_publisher<geographic_msgs::msg::GeoPoseStamped>(ardupilot_namespace_+"setpoint_position/global", 5)},
...
void OffboardControl::timer_callback(void){
	...
	case FlyState::init:
		...
		if (++num_of_steps>10)
			arm_motors(true);
		if(pose_.header.stamp.sec == 0 || arm_done_ == false){
			RCLCPP_INFO(this->get_logger(), "No pose data received yet/not armed");
			break;
		}
	...
	case FlyState::goto_shot_area:
		send_local_setpoint_command(53, 0, 0,0);
