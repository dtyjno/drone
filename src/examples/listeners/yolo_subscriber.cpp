...
public:
    YOLO() : Node("image_pub")
    {
        this->set_parameter(rclcpp::Parameter("use_sim_time", false)); // 确保使用相同的时间源
...
private:
    ...
    {
	if (msg->x1 != x_circle || y_circle != msg->y1){
		// 获取当前时间
            auto current_time = std::chrono::steady_clock::now();
            // 计算时间间隔
            auto time_diff = std::chrono::duration_cast<std::chrono::duration<double>>(current_time - last_time_).count();
            last_time_ = current_time;

            RCLCPP_INFO(this->get_logger(), "收到坐标(%f, %f), flag_servo = %d, 时间间隔 = %f秒", 
                        msg->x1, msg->y1, msg->flag_servo, time_diff);
	}
        x_circle = msg->x1;
        y_circle = msg->y1;
        x_h = msg->x2;
        ...
        //RCLCPP_INFO(this->get_logger(), "收到坐标(%f, %f), flag_servo = %d", x_circle, y_circle, flag);
    }
