...
Vector4f PosControl::input_pos_xyz_yaw(Vector4f now, Vector4f target, bool fuzzy, Vector4b direction)
{
	if(fuzzy){
		...
		float yaw_diff = target.yaw - now.yaw + yaw_diff_last;
		...
		if (direction.yaw)
			fuzzy_pid.fuzzy_pid_control(now.yaw, target.yaw, 3, kp, ki, kd, delta_k);
		else
			fuzzy_pid.fuzzy_pid_control(target.yaw, now.yaw, 3, kp, ki, kd, delta_k);
		pid_yaw.set_gains(kp, ki, kd);
		f.yaw = pid_yaw.update_all(now.yaw, target.yaw, dt, max_speed_yaw, InertialNav::velocity.yaw);
		RCLCPP_INFO(node->get_logger(), "input_pos_vel_xyz_yaw: x:%f y:%f z:%f yaw:%f", f.x, f.y, f.z, f.yaw);
		return f;
